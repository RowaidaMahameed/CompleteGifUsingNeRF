...
def evaluation(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        # Extracting theta, phi, radius from samples
        theta, phi, radius = samples[..., -3], samples[..., -2], samples[..., -1]

        # Print the values of theta, phi, and radius
        print(f"Theta: {theta.cpu().numpy()}, Phi: {phi.cpu().numpy()}, Radius: {radius.cpu().numpy()}")
...
def evaluation_path(test_dataset, tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    near_far = test_dataset.near_far
    print(f'-------- near far: {near_far}')
    for idx, c2w in tqdm(enumerate(c2ws)):

        W, H = test_dataset.img_wh

        c2w = torch.FloatTensor(np.array(c2w, dtype=float))
